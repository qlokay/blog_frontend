      ...
      <el-container class="main">
          ...
          <el-aside width="220px">
            <div class="tag">
              ...
              <ul>
                <li v-for="(item,index) in postData.resTag" :key="item._id" @click="clickHandleLi(index)" :class="{ 'active': activeIndex===index }">{{item.name}}</li>
              </ul>
            </div>
          </el-aside>
      </el-container>
...
export default {
  ...
  methods: {
    ...
    clickHandleLi (index) {
      ...
      this.$router.push('/forum/' + this.postData.resTag[index].name)
    }
  },
  computed: {
    userInfo () {
